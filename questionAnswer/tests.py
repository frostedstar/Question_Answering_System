...
user_list = []
# 老师的邮箱
teacherEmail = "1712269587@qq.com"
...
def userManagement(request):
    ...
    total_users = ReadUserInfo.objects.filter(~Q(user_id=userInfo["userId"]), type="student", approved=1).count()
    total_pages = (total_users + per_page - 1) // per_page

    start_index = (page_number - 1) * per_page
    end_index = start_index + per_page
    page_users = ReadUserInfo.objects.filter(~Q(user_id=userInfo["userId"]), type="student", approved=1).order_by('id')[
...
def registrationDeny(request, user_id):
    user = get_object_or_404(ReadUserInfo, pk=user_id)
    user.delete()
    return redirect(request.META.get('HTTP_REFERER', '/'))
